...
export default function ListThuongHieu() {
...
//code hiển thị danh sách danh mục 
  const getdanhmuc = async() => {
     const base_url = api + 'listthuonghieu/1';
...
const handePageClick = async(data) => {
  let currenPage = data.selected + 1;
    const base_url = api + 'listthuonghieu/' + currenPage;
    const response = await axios.get(base_url)
    setlist_Trademark(response.data);
}
  ...
  return (
      <div className="giao-dien-product">
        <h2 className="Text-Name">Danh Sách Thương Hiệu</h2> 
        <div className="push-2-product">
          <Link to={`/Thuong-Hieu`}>
            <button className="button1">Thương Hiệu Sản Phẩm</button>
          </Link>
