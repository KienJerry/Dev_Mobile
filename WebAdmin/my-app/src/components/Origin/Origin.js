...
//code hiển thị danh sách Xuất Xứ
  const getxuatxu = async() => {
     const base_url = api + 'xuatxu/1';
...
const handePageClick = async(data) => {
  let currenPage = data.selected + 1;
    const base_url = api + 'xuatxu/' + currenPage;
    const response = await axios.get(base_url)
    setlist_Trademark(response.data);
}
  ...
  return (
      <div className="giao-dien-product">
        <h2 className="Text-Name">Nơi Xuất Xứ Sản Phẩm ADMIN</h2> 
        <div className="push-2-product">
          <Link to={`/Them-Noi-Xuat-Xu`}>
            <button className="button1"><img alt="IMG_1" src={require("../../folder_css/img/10.png")} style={{marginRight: 10 ,width : 20 , height: 20 ,}}></img>Thêm Nơi Xuất Xứ</button>
          </Link>
          <Link to={`/List-Noi-Xuat-Xu`}>
            <button className="button2">Danh Sách Xuất Xứ</button>
          </Link>
